...
public class RESTClient
{
	...
	public IEnumerator UploadSession(string customId, Session session, Action<bool, string> callback)
	{
		if (session.GetMovementLabel().Contains(","))
        {
			callback(false, "Caractere inválido no campo \"movementlabel\": ','");
        ...
        {
			string json = TranslationUtility.SessionToJson(session, customId);
			string response = "Request could not be completed properly";
			bool success = false;
